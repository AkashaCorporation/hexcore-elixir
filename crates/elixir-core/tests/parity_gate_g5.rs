...
fn parity_gate_g5_mali_kbase_ko() {
    ...
    println!(
        "G5: Before run: RIP = 0x{:016x}, RSP = 0x{:016x}, RDI = 0x{:016x}",
    ...
    println!(
        "G5: After run: RIP = 0x{:016x}, RSP = 0x{:016x}, RAX = 0x{:016x}",
        rip, rsp, rax
    );

    // Determine if RIP is in stub region (0x72000000+)
    if (0x72000000..0x72100000).contains(&rip) {
    ...
    assert_ne!(
        reason,
        SimpleStopReason::Error,
        "G5 FAILED: fault during execution of kbase_jit_allocate (RIP=0x{:016x})",
        rip
    );

    println!(
        "=== G5 PASSED === (stop_reason={:?}, entry=0x{:x})",
        reason, entry
    );
}
