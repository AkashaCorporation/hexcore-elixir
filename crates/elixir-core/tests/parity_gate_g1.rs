...
fn parity_gate_g1_malware_v1_exits_cleanly() {
    ...
    let binary = std::fs::read("../../tests/fixtures/Malware HexCore Defeat.exe").expect(
        "Failed to read test fixture - ensure tests/fixtures/Malware HexCore Defeat.exe exists",
    );
    ...
    println!(
        "G1: Before run: RIP = 0x{:016x}, RSP = 0x{:016x}",
        rip_before, rsp_before
    );
    ...
    println!(
        "G1: RIP = 0x{:016x}, RSP = 0x{:016x}, RAX = 0x{:016x}",
        rip, rsp, rax
    );

    // Determine if RIP is in stub region (0x70000000+)
    if (0x70000000..0x70100000).contains(&rip) {
    ...
    assert_eq!(
        ...
        "G1 FAILED: Expected clean exit, got {:?}",
        reason
    );
