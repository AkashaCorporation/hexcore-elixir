...
use elixir_core::types::{Arch, MemProt, OsType};
...
fn phase1_mov_rax_ret() {
    ...
    emu.mem_read(0x1000, &mut read_back)
        .expect("Failed to read back shellcode");
    assert_eq!(read_back, shellcode, "Shellcode read-back mismatch");

    // 5. Set registers: rip = 0x1000, rsp = top of stack page
    emu.reg_write(UC_X86_REG_RIP, 0x1000)
        .expect("Failed to set RIP");
    emu.reg_write(UC_X86_REG_RSP, 0x2FF0)
    ...
    emu.mem_write(0x2FF0, &return_addr_bytes)
        .expect("Failed to write return address");
...
fn test_mem_write_read_roundtrip() {
    ...
    emu.mem_map(0x5000, 0x1000, MemProt::RW)
...
fn test_permissive_memory_auto_map() {
    ...
    emu.set_permissive_memory(true)
        .expect("Should enable permissive mode");
    ...
    emu.mem_write(0x2FF0, &0x100E_u64.to_le_bytes())
        .expect("Write return addr");

    // Run - this should NOT crash because permissive mode auto-maps 0x50000
    let result = emu.run(0x1000, 0x100E, 100);
    assert!(
        result.is_ok(),
        "Emulation should succeed with permissive memory: {:?}",
        result
    );

    // Verify the write happened
    let mut buf = [0u8; 4];
    emu.mem_read(0x50000, &mut buf)
        .expect("Should be able to read auto-mapped page");
    assert_eq!(
        u32::from_le_bytes(buf),
        0x42,
        "Value at 0x50000 should be 0x42"
    );
}
