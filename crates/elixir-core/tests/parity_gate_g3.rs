...
fn parity_gate_g3_ashaka_shadow_api_coverage() {
    ...
    emu.set_permissive_memory(true)
        .expect("Failed to set permissive memory");
    ...
    assert!(
        reason == SimpleStopReason::Exit || reason == SimpleStopReason::InsnLimit,
        "G3 FAILED: Unexpected stop reason {:?}",
        reason
    );
    ...
    assert!(
        api_count >= 20_000,
        "G3 FAILED: Only {} API calls (need ≥20,000)",
        api_count
    );
    ...
    println!(
        "G3: Diff from ground truth: {:.1}% ({} vs {})",
        diff_pct, api_count, ground_truth
    );

    assert!(
        diff_pct <= 5.0,
        "G3 FAILED: API count diff {:.1}% exceeds 5% threshold",
        diff_pct
    );

    println!(
        "=== G3 PASSED === ({} API calls, {:.1}% diff from ground truth)",
        api_count, diff_pct
    );
}
