...
fn parity_gate_g4_msvc_hello_world() {
    ...
    println!(
        "G4: Before run: RIP = 0x{:016x}, RSP = 0x{:016x}",
    ...
    println!(
        "G4: RIP = 0x{:016x}, RSP = 0x{:016x}, RAX = 0x{:016x}",
        rip, rsp, rax
    );
    ...
    assert_eq!(
        ...
        "G4 FAILED: Expected clean exit in <100k insns, got {:?}",
        reason
    );
    ...
    println!(
        "=== G4 PASSED === ({} API calls, clean exit in <100k insns)",
        api_count
    );
}
