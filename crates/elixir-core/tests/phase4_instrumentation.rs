...
fn test_stalker_traces_basic_blocks() {
    ...
    emu.mem_write(0x2FF8, &ret_addr.to_le_bytes())
        .expect("write ret");
    ...
    assert!(
        blocks > 0,
        "Stalker should have traced at least 1 block, got {}",
        blocks
    );
...
fn test_drcov_export_has_valid_header() {
    ...
    emu.mem_write(0x2FF8, &0x1008u64.to_le_bytes())
        .expect("ret");
    ...
    assert!(
        header.starts_with("DRCOV VERSION: 2"),
        "DRCOV should start with version header, got: {}",
        header
    );
...
fn test_snapshot_save_restore_roundtrip() {
    ...
    assert_eq!(
        &snapshot[0..7],
        b"ELXSNAP",
        "Snapshot should start with ELXSNAP magic"
    );
    ...
    let restored_rax = emu
        .reg_read(UC_X86_REG_RAX)
        .expect("read rax after restore");
    assert_eq!(
        restored_rax, 0x1234,
        "RAX should be restored to original value"
    );
...
fn test_interceptor_attach_and_log() {
    ...
    assert!(
        log_count > 0,
        "Interceptor should have logged at least 1 call, got {}",
        log_count
    );
